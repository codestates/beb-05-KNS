...
module.exports = {
    ...
    updatePost : asyncWrapper(async (req, res) => {
        ...
        if (!postData) {
            //404 not found
            throw new CustomError(`글번호 ${req.params.id} 번이 존재하지 않습니다.`, StatusCodes.NOT_FOUND);
        }
    ...
    likePost : asyncWrapper(async (req, res) => {
        ...
        await post.increment({like: 1}, {where: {id: req.params.id}}) // 게시글 좋아요 증가
    ...
    getPostById: asyncWrapper(async (req, res) => {
        ...
        await post.increment({hit: 1}, {where: {id: req.params.id}}) // 게시글 상세 조회하면 조회수 증가
    ...
    writeComment : asyncWrapper(async (req, res) => {
        if (req.params.id === undefined || req.body.content === undefined) {
            throw new CustomError("올바르지 않은 파라미터 값입니다.",StatusCodes.BAD_REQUEST);
        }
    ...
    deleteComment : asyncWrapper(async (req, res) => {
      ...
      await postData.remove(commentId);
      res.sendStatus(StatusCodes.NO_CONTENT); //204 
    }),
